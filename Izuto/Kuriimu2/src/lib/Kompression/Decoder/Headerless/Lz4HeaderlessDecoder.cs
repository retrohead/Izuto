...
namespace Kompression.Decoder.Headerless
{
    class Lz4HeaderlessDecoder : IDecoder
    {
        ...
        public void Decode(Stream input, Stream output)
        {
            ...
            while (input.Position < input.Length && !IsLastBlock(decompSize))
            {
                ...
                while (input.Position < blockPosition + 8 + compSize)
                {
                    ...
                    _ = input.Read(literalBuffer, 0, literalLength);
        ...
        {
            _ = input.Read(_buffer);
            decompBlockSize = BinaryPrimitives.ReadInt32LittleEndian(_buffer);

            _ = input.Read(_buffer);
            int compSize = BinaryPrimitives.ReadInt32LittleEndian(_buffer);
