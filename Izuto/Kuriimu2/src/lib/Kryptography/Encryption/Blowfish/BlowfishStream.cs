...
namespace Kryptography.Encryption.Blowfish
{
    public class BlowfishStream : Stream
    {
        ...
        public BlowfishStream(Stream baseStream, byte[] key)
        {
            SetupKey(key);
            _baseStream = baseStream;

            _internalLength = baseStream.Length;
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            if (decryptCount > 0) Decrypt(internalBuffer, (int)decryptCount);
        ...
        private void InternalRead(byte[] buffer, int offset, int count)
        {
            ...
            Decrypt(internalBuffer, internalBuffer.Length);
        ...
        /// <summary>
        /// Sets up the S-blocks and the _key
        /// </summary>
        /// <param name="cipherKey">Block cipher _key (1-448 bits)</param>
        private void SetupKey(byte[] cipherKey)
        {
            if (cipherKey.Length > 56)
                throw new InvalidOperationException("Key too long. 56 bytes required.");
            ...
            // Setup pi
            var j = 0;
            for (var i = 0; i < 18; i++, j += 4)
            {
                var d = (uint)(_key[j % cipherKey.Length] << 24 | _key[(j + 1) % cipherKey.Length] << 16 |
                         _key[(j + 2) % cipherKey.Length] << 8 | _key[(j + 3) % cipherKey.Length]);
                _pi[i] ^= d;
            }
        ...
        {
            var x1 = _s0[b >> 24 & 0xFF] + _s1[b >> 16 & 0xFF] ^ _s2[b >> 8 & 0xFF];
            var x2 = x1 + _s3[b & 0xFF];
            var x3 = x2 ^ _pi[n];
        ...
        /// <summary>
        /// Decrypts an arbitrary buffer of data.
        /// </summary>
        /// <param name="input">The buffer to decrypt.</param>
        /// <param name="length">The amount of data to decrypt.</param>
        private void Decrypt(byte[] input, int length)
        {
            var block = new byte[BlockSize];
            for (var i = 0; i < length; i += 8)
            {
                Array.Copy(input, i, block, 0, BlockSize);
                BlockDecrypt(block);
                Array.Copy(block, 0, input, i, BlockSize);
            }
        }
        ...
        private void Decipher()
        {
            ...
            {
                _right = Round(_right, _left, i);
                _left = Round(_left, _right, i - 1);
            }
            _right ^= _pi[0];
        ...
        /// <param name="input">The buffer to decrypt.</param>
        /// <param name="length">The amount of data to decrypt.</param>
        private void Encrypt(byte[] input, int length)
        {
            var block = new byte[BlockSize];
        ...
        private void Encipher()
        {
            ...
            {
                _right = Round(_right, _left, i + 1);
                _left = Round(_left, _right, i + 2);
            }
            _right ^= _pi[17];
        ...
        // HINT: S-Blocks are hex digits of Pi. 
        // HINT: The amount of hex digits can be increased if one wants to experiment with more rounds and longer _key lengths.
        private uint[] SetupP()
        {
            return new uint[] {
