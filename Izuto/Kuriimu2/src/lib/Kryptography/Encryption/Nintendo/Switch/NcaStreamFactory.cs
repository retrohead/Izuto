...
using Kryptography.Encryption.Nintendo.Switch.KeyStorages;
using Kryptography.Encryption.Nintendo.Switch.Models;
using Kryptography.Encryption.Nintendo.Switch.Streams;

namespace Kryptography.Encryption.Nintendo.Switch
{
    /// <summary>
    ...
    public class NcaStreamFactory
    {
        ...
        public Stream CreateStream(Stream baseStream)
        {
            ...
            if (UseTitleKeyEncryption)
            {
                ...
                if (EncryptedTitleKey == null)
                {
                    ...
                    Array.Copy(_titleKeyStorage[Convert.ToHexString(TitleId)], EncryptedTitleKey, 0x10);
                }
                else
                {
        ...
        private void IdentifyEncryptedTitleKey(Stream header)
        {
            ...
            {
                EncryptedTitleKey = new byte[0x10];
                Array.Copy(_titleKeyStorage[Convert.ToHexString(rightsId)], EncryptedTitleKey, 0x10);
            }
        }
