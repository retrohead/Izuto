...
namespace Izuto.Extensions
{
    public class hexAndMathFunctions
    {
        ...
        public static string halfByteSwap(string hex)
        {
            ...
            {
                App.CustomMessageBox.Show("Trying to halfByte swap an uneven (" + (object)hex.Length + ") amount of bytes!");
                return "";
            }
        ...
        public static string convertHexToEncryptionKey(string hex)
        {
            if (hex.Length < 16)
            {
                App.CustomMessageBox.Show("The application encoded key is incorrect");
                Environment.Exit(0);
            }
            string str = "";
